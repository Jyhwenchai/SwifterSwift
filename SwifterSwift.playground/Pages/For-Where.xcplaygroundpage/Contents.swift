...
print(" \n ======== \n")
//: 我们使用 `where` 能够很好的简话前面的示例，并且易于阅读
...
//: 使用 `for [case let] in [where]`
//: 在前面的示例演示了 `for-[case let]-in` 的基础上我们加入 `where` 以对遍历中的数据进一步筛选
for case let .some(value) in optionalArray where value > 5 {
